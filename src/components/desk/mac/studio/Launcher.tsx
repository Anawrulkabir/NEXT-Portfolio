...
function Chips<T extends string | number | boolean>({ label, value, options, onChange }: { label: string; value: T; options: [T, string][]; onChange: (v: T) => void }) {
  return (
    <fieldset>
      <legend className="mb-1.5 text-[11px] font-semibold uppercase tracking-wider text-white/45">{label}</legend>
      <div className="flex flex-wrap gap-1.5">
        {options.map(([v, text]) => (
          <button
            ...
            onClick={() => onChange(v)}
            className={`rounded-lg border px-3 py-1.5 text-[12.5px] ${value === v ? 'border-[#efe6d6] bg-[#efe6d6] font-medium text-black' : 'border-white/15 hover:border-white/40'}`}
          >
            {text}
          </button>
        ))}
  ...
  if (phase === 'ready') {
    ...
    return (
      <div className="flex h-full min-h-0 flex-col">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 bg-[#0f1a14] px-3 py-1.5 text-[12px] text-white/75">
          <span className="font-semibold text-[#7fc08c]">● session running</span>
          <span>{o.gpu ? 'RTX 4090 · 8 GB slice' : 'CPU only'}</span>
          <span>
            {o.cpu} CPU · {o.mem} GB
          </span>
          <span className="tabular-nums">ends in {left}</span>
          <span className="text-white/40">demo, runs in your browser</span>
          <button type="button" onClick={end} className="ml-auto rounded-md bg-white/10 px-2.5 py-0.5 hover:bg-[#a9444c]">
            End session
          </button>
        </div>
    ...
    <div className="grid h-full min-h-0 grid-cols-1 md:grid-cols-[1fr_340px]">
      ...
      >
        <div>
          ...
          <p className="text-[12.5px] text-white/55">Pick what you need. This mirrors how AI Studio sessions are requested; it isn’t the real interface.</p>
        </div>
        ...
        <button type="submit" disabled={phase !== 'form'} className="mt-1 self-start rounded-lg bg-[#efe6d6] px-6 py-2 font-semibold text-black hover:bg-white disabled:opacity-60">
          {phase === 'form' ? 'Launch session' : 'Launching…'}
        </button>
      </form>

      <aside className="flex flex-col gap-3 border-t border-white/10 p-5 md:border-l md:border-t-0">
        <p className="text-[11px] font-semibold uppercase tracking-wider text-white/45">Your request</p>
        <ul className="space-y-1.5 text-[13px]">
          <li className="flex items-center gap-2">
            <HardDrive className="h-4 w-4 text-white/50" aria-hidden="true" /> {o.gpu ? 'One 8 GB slice of an RTX 4090' : 'No GPU'}
          </li>
          <li className="flex items-center gap-2">
            <Cpu className="h-4 w-4 text-white/50" aria-hidden="true" /> {o.cpu} CPU cores
          </li>
          <li className="flex items-center gap-2">
            <MemoryStick className="h-4 w-4 text-white/50" aria-hidden="true" /> {o.mem} GB memory
          </li>
          <li className="flex items-center gap-2">
            <Timer className="h-4 w-4 text-white/50" aria-hidden="true" /> {o.hours} hour{o.hours > 1 ? 's' : ''}, {o.ide === 'jupyter' ? 'JupyterLab' : 'VS Code'}
          </li>
        </ul>
        {phase === 'launching' && (
          <ol className="mt-2 space-y-2 text-[12.5px] leading-snug" aria-live="polite">
            {visible.map((s, i) => (
              <li key={i} className={`flex gap-2 ${i < step ? 'text-white/55' : i === step ? 'text-white' : 'text-white/25'}`}>
                <span className={`mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full text-[10px] ${i < step ? 'bg-[#7fc08c] text-black' : i === step ? 'bg-[#f2c14e] text-black' : 'bg-white/10'}`}>
        ...
        <p className="mt-auto text-[11.5px] text-white/40">In real life this takes 3–5 minutes on the bare-metal cluster. Here it takes a few seconds.</p>
      </aside>
    </div>
  )
}
